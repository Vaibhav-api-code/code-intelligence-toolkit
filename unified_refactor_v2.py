...
"""
Unified Refactoring Tool (v2) - Comprehensive AST-based refactoring with multi-level undo support.

This version adds SafeGIT-style multi-level undo capabilities to all refactoring operations.
...
class AtomicFileOperations:
    ...
    def write_file_atomic(self, file_path: str, content: str, encoding: str = 'utf-8') -> bool:
        """Write file atomically with retry logic using secure temp file creation and undo support (v2)."""
        global _undo_enabled, _operations_tracked, _undo_description
        
        # Track operation for undo if enabled (v2 feature)
        if HAS_UNDO_SYSTEM and _undo_enabled and os.path.exists(file_path):
            try:
                # Only track if we haven't already tracked this file in this session
                file_key = str(file_path)
                if file_key not in _operations_tracked:
                    original_content = Path(file_path).read_text(encoding=encoding)
                    ...
                    if _undo_description:
                        ...
                        description = "Unified refactoring operation"
                        if len(cmd_args) >= 2:
                            description = f"Unified refactor: {cmd_args[0]} → {cmd_args[1]}"
                    
                    # Record operation
                    history = get_history_instance()
                    operation = history.record_operation(
                        operation_type=OperationType.UNIFIED_REFACTOR,
                        file_path=Path(file_path),
                        tool_name="unified_refactor_v2",
...
def create_parser() -> argparse.ArgumentParser:
    ...
    # Undo system arguments (v2 feature)
...
def main():
    ...
    try:
        ...
        # Show undo summary if operations were tracked (v2 feature)
        if HAS_UNDO_SYSTEM and _operations_tracked and not args.dry_run:
            print(f"\n✅ Tracked {len(_operations_tracked)} refactoring operation(s) with undo support.")
