...
usage() {
    ...
    echo "    replace_text.py - Definitive text replacement tool (V7 consolidated) with ripgrep integration"
    echo "                     ALL FEATURES: Block-aware replacement, JSON pipeline support, blazing fast file discovery"
    echo "                     (ripgrep search, AST context display, find→replace workflows, multi-strategy replacement)"
    ...
    echo ""
    echo "replace_text.py examples (DEFINITIVE V7 - ALL VERSIONS CONSOLIDATED):"
    echo "  ⚠️  IMPORTANT: Order is <old_text> <new_text> <paths>"
    echo "  $0 replace_text.py oldMethod newMethod MyClass.java --whole-word    # Safe variable renaming"
    echo "  $0 replace_text.py 'TODO' 'DONE' . -r -g '*.java' --dry-run        # Multi-file replacement preview"
    echo "  $0 replace_text.py oldValue newValue . -r --git-only -g '*.py'     # Only Git-tracked files"
    echo "  $0 replace_text.py old new . -r --staged-only -g '*.java'          # Only staged files"
    echo "  # CONSOLIDATED: JSON pipeline support for find→replace workflows"
    echo "  $0 replace_text.py --from-find-json search_results.json old new    # Use find_text results"
    echo "  # CONSOLIDATED: Block-aware replacement modes"
    echo "  $0 replace_text.py old new MyClass.java --block-mode within         # Replace only within code blocks"
    echo "  $0 replace_text.py old new MyClass.java --block-mode extract        # Extract and replace in blocks"
    echo "  # CONSOLIDATED: AST context display during replacement"
    echo "  $0 replace_text.py calculateValue computeResult MyClass.java --ast-context  # Show class/method context"
    echo "  # ALL traditional features included:"
    echo "  $0 replace_text.py 'TODO:.*' 'TODO: Updated' Config.java --regex --comments-only"
    echo "  $0 replace_text.py value formattedValue File.java --whole-word        # Avoids value→formattedValueInUnits"
    echo ""
    ...
    echo "   $0 replace_text.py --from-find-json search_results.json calculateTotal computeTotal"
    echo ""
    ...
    echo "   $0 replace_text.py --from-find-json blocks.json --block-mode within validateInput checkInput"
    echo ""
    ...
    echo "  • Text-based tools (find_text.py, replace_text.py, etc.) - All languages"
    ...
    echo "  ❌ WRONG: replace_text.py file.java 'old' 'new'     # File path should be LAST"
    echo "  ✅ RIGHT: replace_text.py 'old' 'new' file.java     # Correct order"
    echo ""
...
case "$TOOL_NAME" in
    ...
    "find_text.py")
        ...
        echo "ℹ️  Using find_text_v6.py - COMPLETE VERSION with ALL features: block/method/range extraction, wholefile mode, AST context, auto-find, and more"
        ;;
    ...
    "show_structure_ast.py")
        ...
        ;;
    "replace_text.py")
        TOOL_NAME="replace_text_v7.py"
        echo "ℹ️  Using replace_text_v7.py - DEFINITIVE text replacement tool with all V7 features consolidated"
        ;;
    "replace_text_ast.py")
...
if [[ "$TOOL_NAME" == "replace_text_v7.py" && $# -eq 0 ]]; then
    echo "Quick Start Guide for replace_text_v7.py (DEFINITIVE CONSOLIDATED VERSION):"
    echo "=========================================================================="
    echo ""
    ...
    echo "   $0 replace_text.py oldMethod newMethod MyClass.java --whole-word"
    echo ""
    echo "2. Multi-file replacement with preview:"
    echo "   $0 replace_text.py \"old text\" \"new text\" . -r -g '*.java' --dry-run"
    echo ""
    echo "3. JSON pipeline workflow (find→replace):"
    echo "   $0 find_text.py \"TODO\" --scope src/ --json > results.json"
    echo "   $0 replace_text.py --from-find-json results.json \"TODO\" \"DONE\""
    echo ""
    echo "4. Block-aware replacement (replace only within code blocks):"
    echo "   $0 replace_text.py calculateTotal computeTotal MyClass.java --block-mode within"
    echo ""
    echo "5. Extract and replace in code blocks:"
    echo "   $0 replace_text.py old new MyClass.java --block-mode extract"
    echo ""
    echo "6. AST context display during replacement:"
    echo "   $0 replace_text.py processData handleData Service.java --ast-context"
    echo ""
    echo "7. Git-integrated replacement (only tracked files):"
    echo "   $0 replace_text.py oldValue newValue . -r --git-only -g '*.py'"
    echo ""
    echo "8. Traditional features (still supported):"
    echo "   $0 replace_text.py \"TODO\" \"DONE\" . -r --comments-only -g '*.java'"
    echo "   $0 replace_text.py value formattedValue MyClass.java --whole-word"
    echo ""
    echo "CONSOLIDATED V7 FEATURES (ALL PREVIOUS VERSIONS UNIFIED):"
    echo "• Ripgrep integration - blazing fast file discovery and pattern searching"
    echo "• JSON pipeline support - seamless find→replace workflows"
    echo "• Block-aware replacement - preserve/within/extract modes for structural code modification"
    echo "• AST context display - show class/method hierarchy during replacement"
    echo "• Enhanced robustness - cross-platform support with Windows drive letter handling"
    echo "• Safe pattern handling - automatic -e flag usage and -- separator"
    echo "• Complete feature consolidation - all replace_text versions unified into this definitive tool"
    echo ""
    echo "For full help: $0 replace_text.py --help"
    echo ""
    exit 0
fi
