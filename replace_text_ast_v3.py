...
"""
Enhanced AST-based text replacement tool (v3) with multi-level undo support.

This version adds SafeGIT-style multi-level undo capabilities to all AST-based refactoring.
Every operation is tracked with atomic operations and full recovery support.
...
# ────────────────────────────  logging  ────────────────────────────
LOG = logging.getLogger("replace_text_ast_v3")
logging.basicConfig(
    level=os.getenv("LOG_LEVEL", "INFO").upper(),
    format="%(asctime)s - %(name)s - %(levelname)s: %(message)s"
)

# Import undo system (v3 feature)
try:
...
try:
    ...
    def get_multi_choice(prompt, choices, **kwargs):
        choice_str = "/".join([k for k, _ in choices])
        ...
        try:
            ...
            if confirm_each and not auto_yes:
                ...
                if HAS_INTERACTIVE_UTILS:
                    choices = [('y', PromptChoice.YES), ('n', PromptChoice.NO), ('q', PromptChoice.QUIT)]
                    choice = get_multi_choice(
                        f"\nRename '{old_name}' to '{new_name}' in this file?",
                        choices,
                        default=PromptChoice.NO,
                        tool_name='replace_text_ast',
                        env_var='REPLACE_TEXT_AST_ASSUME_YES',
                        flag_hint='--yes or --no-confirm'
                    )
                    if choice == PromptChoice.QUIT:
                        print("Batch operation cancelled.")
                        break
                    elif choice != PromptChoice.YES:
                        print("Skipping file.")
                        continue
                else:
...
def _atomic_write(path: Path, data: str, bak: bool = False, max_retries: int = 3, retry_delay: float = 1.0) -> None:
    """Write atomically with retry logic for locked files and undo support (v3)."""
    global _undo_enabled, _operations_tracked, _undo_description
    
    # Track operation for undo if enabled (v3 feature)
        ...
        try:
            ...
            if file_key not in _operations_tracked:
                ...
                if _undo_description:
                    ...
                    # For AST operations, try to get more context
                    description = "AST-based refactoring"
                    if len(cmd_args) >= 2:
                        if "--line" in cmd_args:
                            description = f"AST refactoring at line {cmd_args[cmd_args.index('--line') + 1]}"
                        else:
                            description = f"AST replace '{cmd_args[0][:30]}...' with '{cmd_args[1][:30]}...'"
                
                # Record operation
                history = get_history_instance()
                operation = history.record_operation(
                    operation_type=OperationType.REPLACE_AST,
                    file_path=path,
                    tool_name="replace_text_ast_v3",
...
Environment Variables:
  ...
  REPLACE_TEXT_AST_NONINTERACTIVE   # Default: false - Run in non-interactive mode
    ...
    # Undo system arguments (v3 feature)
    ...
    if HAS_INTERACTIVE_UTILS:
        check_auto_yes_env('replace_text_ast', args)
    else:
        # Fallback to manual environment variable handling
        if os.getenv('REPLACE_TEXT_AST_NONINTERACTIVE') == '1':
            args.non_interactive = True
        if os.getenv('REPLACE_TEXT_AST_ASSUME_YES') == '1':
            args.yes = True
        ...
        if args.batch_rename:
            ...
            if HAS_INTERACTIVE_UTILS:
                confirmed = get_confirmation(
                    f"\nProceed with batch rename '{args.old_name}' → '{args.new_name}'?",
                    default=False,
                    tool_name='replace_text_ast',
                    env_var='REPLACE_TEXT_AST_ASSUME_YES',
                    flag_hint='--yes'
    ...
    # Show undo summary if operations were tracked (v3 feature)
    if HAS_UNDO_SYSTEM and _operations_tracked and not args.quiet:
        print(f"\n✅ Tracked {len(_operations_tracked)} AST refactoring operation(s) with undo support.")
