...
def cmd_undo(args):
    ...
    if args.interactive:
        ...
        for i, op in enumerate(undoable, 1):
            file_name = Path(op.file_path).name
            print(f"{i:2}. [{op.operation_id}] {format_timestamp(op.timestamp)} - "
                  f"{file_name} ({op.changes_count} changes)")
        
        try:
            choice = input(f"\nSelect operation to undo (1-{len(undoable)}) or 'q' to quit: ")
            if choice.lower() == 'q':
                return 0
            
            idx = int(choice) - 1
            if 0 <= idx < len(undoable):
