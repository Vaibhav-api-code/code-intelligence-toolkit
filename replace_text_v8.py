▶ safegit.py show HEAD~1:code-intelligence-toolkit/replace_text_v8.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
def get_files_to_process_legacy(paths, glob_pattern, git_only, staged_only, language=None):
    ...
    if git_only or staged_only:
        ...
        for path_str in paths:
            ...
            if path.is_dir():
                for file_path in path.rglob(glob_pattern or "*"):
                    if file_path.is_file():
                        if should_include_file(file_path, language, language_extensions):
                            files_to_process.add(file_path)
            elif path.is_file():
                if glob_pattern is None or glob_pattern == "*" or path.match(glob_pattern):
...
def main():
    ...
    # Note: -g/--glob is already defined in the parser section above when not HAS_ENHANCED_PARSER
    ...
    # Note: -i/--ignore-case is already defined in the parser section above
    
    parser.add_argument('--multiline', '-m', action='store_true', help='Multiline mode for regex')
    parser.add_argument('--interpret-escapes', action='store_true', 
                       help='Interpret escape sequences in replacement text (\\n, \\t, \\r, \\\\)')
    ...
    if hasattr(args, 'interpret_escapes') and args.interpret_escapes:
        args.new_text = interpret_escape_sequences(args.new_text)
        ...
        if not args.paths or (len(args.paths) == 1 and args.paths[0] == '-'):
            ...
            if args.use_ripgrep and HAS_RIPGREP_INTEGRATION:
                try:
                    ...
                    files_to_process = get_files_to_process_legacy(args.paths, getattr(args, 'glob', '*'), args.git_only, args.staged_only, args.language)
            else:
                files_to_process = get_files_to_process_legacy(args.paths, getattr(args, 'glob', '*'), args.git_only, args.staged_only, args.language)
