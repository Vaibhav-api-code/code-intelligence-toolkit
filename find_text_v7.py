...
"""
find_text v7 - Enhanced text search with structural code block extraction and multiline support.
...
def search_with_context(pattern: str, args, context_lines: int = 0, extract_block: bool = False) -> List[Dict]:
    ...
    if hasattr(args, 'multiline') and args.multiline:
        cmd.append('-U')  # Enable multiline mode
        cmd.append('--multiline-dotall')  # Make . match newlines
...
"""
    ...
    # Multiline search support (v7)
    parser.add_argument('--multiline', '-U', action='store_true',
                      help='Enable multiline mode where . matches newlines and patterns can span lines')
