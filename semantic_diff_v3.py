...
def main():
    """Main entry point"""
    from standard_arg_parser import StandardArgumentParser
    
    # Create a custom parser - don't use analyze pattern as it expects different arguments
    parser = argparse.ArgumentParser(description="Semantic Diff V3 - Advanced semantic code analysis")
    
    # Add verbose/quiet always since we're not using standard patterns
    # Primary input arguments for file comparison
    parser.add_argument('file1', help='First file or directory to compare')
    parser.add_argument('file2', help='Second file or directory to compare')
    ...
    # Validate inputs
    if args.git:
        if hasattr(args, 'file1') or hasattr(args, 'file2'):
            parser.error("Cannot specify files with --git option")
    elif not (args.file1 and args.file2):
    ...
    try:
        ...
        elif os.path.isdir(args.file1) and os.path.isdir(args.file2):
            report = analyzer.analyze_directory(args.file1, args.file2, args.extensions)
        else:
            report = analyzer.analyze_files(args.file1, args.file2)
        
        # Additional analysis if requested
        if args.test_impact:
            test_impact = analyzer.test_analyzer.analyze_test_impact(
                report.changes, 
                os.path.dirname(args.file1) if not args.git else '.'
            )
